...
class ProcessVideoUpload implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            if ($this->platformUpload->platform === 'youtube') {
                ...
                $credential = UserPlatformCredential::where('user_id', $job->user_id)
                ...
                if ($job->thumbnail_path) {
                    try {
                        ...
                        if ($readThumbStream) {
                            ...
                            $mediaThumb = new MediaFileUpload(
                                $client,
                                $setRequest,
                                'image/*',
                                null,
                ...
                $response = Http::timeout(3600)->withToken($pageToken)
                ...
                if (! $creationId) {
                    $containerResponse = Http::withToken($pageToken)
                ...
                $statusCheck = Http::withToken($pageToken)
                ...
                $publishResponse = Http::withToken($pageToken)
                ...
                if (! $publishId) {
                    ...
                    $initResponse = Http::withToken($token)
                        ->post('https://open.tiktokapis.com/v2/post/publish/video/init/', [
                            'post_info' => [
                                'title' => $job->title,
                                'privacy_level' => 'PUBLIC_TO_EVERYONE', // Diubah dari SELF_ONLY ke PUBLIC_TO_EVERYONE agar bisa diposting secara publik setelah aplikasi disetujui (Live)
                                'disable_duet' => false,
    ...
    protected function cleanUpCache(): void
    {
        ...
        $remaining = PlatformUpload::where('job_id', $job->id)
            ...
            try {
                ...
                if ($localDisk->exists($job->file_path)) {
                    ...
                    if ($localStream) {
                        ...
                        $uploaded = Storage::disk($cloudDiskName)->writeStream($cloudPath, $localStream);
                ...
                if ($job->thumbnail_path && $localDisk->exists($job->thumbnail_path)) {
                    ...
                    if ($localThumbStream) {
                        ...
                        $uploadedThumb = Storage::disk($cloudDiskName)->writeStream($cloudThumbPath, $localThumbStream);
